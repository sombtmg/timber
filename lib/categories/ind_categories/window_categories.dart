...
import 'package:timber/categories/ind_categories/list/window_products.dart';
import 'package:timber/media/color.dart';
class WindowCategories extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: productBar(context,"Window Category"),
      body: WindowCategory(),
    );
  }
}

class WindowCategory extends StatefulWidget {
  @override
  _WindowCategoryState createState() => _WindowCategoryState();
}

class _WindowCategoryState extends State<WindowCategory> {
  List recentProduct = [
    {
      'name': 'Window 1',
  ...
  Widget build(BuildContext context) {
    return Card(
      child: Material(
          child: InkWell(
            onTap: () {
              Navigator.of(context).push(new MaterialPageRoute(builder: (context)=> WindowProducts()));
